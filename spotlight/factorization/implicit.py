...
class ImplicitFactorizationModel(object):
    ...
    def __init__(self,
        ...
        self._neg_samples = _neg_samples
        self._num_users = None
        self._num_items = None
        self._net = None
        self._optimizer = None
        self._loss_func = None
        self.use_noloss = use_noloss
    ...
    def fit(self, interactions, verbose=False):
        ...
        for epoch_num in range(self._n_iter):
            ...
            for (minibatch_num,
                ...
                positive_prediction, w_pos = self._net(batch_user, batch_item)
                negative_prediction, w_neg = self._get_negative_prediction(batch_user)
                ...
                if self.cus_loss is not None:
                    negs, (u, i, b) = self._get_multiple_negative_predictions(batch_user, n=self._neg_samples)
                    reg += (i ** 2).sum()
                    reg += (b ** 2).sum()

                    cl = self.cus_loss(positive_prediction, negs)
                    loss += cl
                    cus_losses += cl.item()

                (loss + self._l2 * reg).backward()
                self._optimizer.step()

            epoch_loss /= minibatch_num + 1

            if verbose:
                if self.cus_loss is None:
                    print('Epoch {}: loss {}'.format(epoch_num, epoch_loss))
                else:
                    print('Epoch {}: loss {}\t NeuralLoss {}'.format(epoch_num, epoch_loss, cus_losses / (minibatch_num+ 1 )))
    ...
    def _get_negative_prediction(self, user_ids):
        ...
        negative_prediction, w_neg = self._net(user_ids, negative_var)

        return negative_prediction, w_neg

    def _get_multiple_negative_predictions(self, user_ids, n=5):

        batch_size = user_ids.size(0)

        negative_prediction, x = self._get_negative_prediction(user_ids
                                                            .view(batch_size, 1)
                                                            .expand(batch_size, n)
                                                            .reshape(batch_size * n))

        return negative_prediction.view(n, len(user_ids)), x
    ...
    def predict(self, user_ids, item_ids=None):
        ...
        out, _ = self._net(user_ids, item_ids)

        return cpu(out).detach().numpy().flatten()

    def rank_items(self, userid, tr, item_ids):
        scores = self.predict(userid, item_ids)
        order = (-scores).argsort()
        s = scores[order].tolist()
        i = item_ids[order].tolist()
        return list(zip(i, s))
...
class ImplicitFactorizationModel(object):
    ...
    def _initialize(self, interactions):
        ...
        self.net = gpu(CML(self._num_users, self._num_items))
        self._optimizer = optim.Adam(self._net.parameters(), lr=self._learning_rate)
    ...
    def fit(self, interactions, verbose=False):
        ...
        for epoch_num in range(self._n_iter):
            ...
            for (minibatch_num,
                ...
                positive_prediction, (u_emb, i_emb) = self._net(batch_user, batch_item)
                negative_predictions, (_, j_embs) = self._get_multiple_negative_predictions(batch_user)

                tot_embs = torch.cat([u_emb, i_emb, j_emb], dim=0)
                ...
                if self.cus_loss is not None:
                    negs, (u, i, b) = self._get_multiple_negative_predictions(batch_user, n=self.neg_samples)
                    reg += (i ** 2).sum()
                    reg += (b ** 2).sum()
