...
def NeuralSort(scores, tau=1.0, hard=False):
    """
    ...
    bsize = scores.size()[0]
    dim = scores.size()[1]
    one = torch.FloatTensor(dim, 1).fill_(1)
    ...
    if hard:
        ...
        b_idx = torch.arange(bsize).repeat([1, dim]).view(dim, bsize).transpose(
            dim0=1, dim1=0).flatten().type(torch.LongTensor)
        r_idx = torch.arange(dim).repeat(
            [bsize, 1]).flatten().type(torch.LongTensor)
