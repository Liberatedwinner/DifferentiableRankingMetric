    ...
    def rsample(self, sample_shape, log_score=True):
        ...
        with torch.enable_grad():  # torch.distributions turns off autograd
            ...
            def sample_gumbel(samples_shape, eps=1e-20):
                U = torch.zeros(samples_shape).uniform_().cuda()
...
if __name__ == '__main__':
    ...
    scores = torch.Tensor([[1,2,3], [100.8, 0.3, 11111.9]]).unsqueeze(-1)
    tau = 0.1
    ...
    print(sorted_scores)
    q = torch.matmul(sorted_scores.type(torch.LongTensor), torch.arange(3))
